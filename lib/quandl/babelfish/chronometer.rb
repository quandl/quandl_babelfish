    ...
    class Chronometer
      class << self
        ...
        def process(table)
          ...
          return nil,nil if table.nil? || table.size==0
          return 'daily','defaulted to \'daily\' since only one date point is available' if table.size==1      #not enough , need more points
          freqs = []
          fmt = "%Y-%m"
          ...
          freqs.sort_by { |e| freqs.count(e) }.reverse.first
        end
      end
    end
